...
PATH_TO_SAVED_MODEL = 'models/' + MODEL_NAME + '/saved_model'
...
# Get input name from command line arguments
args = sys.argv
if len(args) > 1:
    input_name = args[1]
...
while cap.isOpened():
    ...
    if ret:
        # Expand dimensions since the model expects images to have shape: [1, X, X, 3]
        input_tensor = np.expand_dims(frame, axis=0)

        # Object detection
        detections = model(input_tensor)

        # In-place adding detection items on frame
        vis_utils.visualize_boxes_and_labels_on_image_array(
            frame,
            detections['detection_boxes'][0].numpy(),
            detections['detection_classes'][0].numpy().astype(np.int8),
        ...
        # Save frame with detection items
        out.write(frame)
    else:
        break
